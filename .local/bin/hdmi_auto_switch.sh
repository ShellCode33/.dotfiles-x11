...
#
#        ACTION=="change", SUBSYSTEM=="drm", ENV{DISPLAY}=":0", ENV{XAUTHORITY}="/home/USERNAME/.Xauthority", RUN+="/usr/bin/su USERNAME -c /path/to/hdmi_auto_switch.sh"
#
...
if [ -n "$SECONDARY_DISPLAY" ]; then
    ...
    notify-send 'HDMI output detected'
else
    PROFILE="analog-stereo"
    xrandr --auto
    notify-send 'HDMI cable plugged out'
fi
